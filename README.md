...
首先我透過動畫瘋提供的 `sitemap.xml` 取得原始數據，並透過 Docker Compose 將資料儲存到本地的 PostgreSQL 資料庫，接著在資料庫上進行兩層清洗，得到 `v_clean_tv_animelist` 這個 View。再利用此 View 整理出的連結，透過爬蟲抓取 Sitemap 上沒有提供的資料（代理廠商、製作廠商、分類標籤），並將結果存回資料庫成 `anime_metadata` 表，同時為製作公司建立三張正規化表。最後拿這些數據進一步做非監督式機器學習（降維與分群），並製作 Power BI 儀表板以呈現數據。
